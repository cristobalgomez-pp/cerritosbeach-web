...
describe('createRestaurant', () => {
  ...
  it('returns UNAUTHORIZED when not admin', async () => {
    ...
    const result = await createRestaurant(VALID_INPUT);
  ...
  it('returns VALIDATION error when required fields are missing', async () => {
    ...
    const result = await createRestaurant({ slug: '', name_es: '', name_en: '' });
  ...
  it('returns VALIDATION error when slug has invalid characters', async () => {
    ...
    const result = await createRestaurant({ slug: 'El Nido', name_es: 'x', name_en: 'x' });

    expect(result).toMatchObject({ status: 'error', code: 'VALIDATION' });
  });

  it('inserts restaurant and returns success when valid and admin', async () => {
    mockedRequireAdmin.mockResolvedValue(GUARD_OK);
...
describe('updateRestaurant', () => {
  ...
  it('returns UNAUTHORIZED when not admin', async () => {
    ...
    const result = await updateRestaurant('some-id', { name_es: 'Nuevo' });
  ...
  it('returns VALIDATION error when slug has invalid characters', async () => {
    ...
    const result = await updateRestaurant('some-id', { slug: 'HAS SPACES' });

    expect(result).toMatchObject({ status: 'error', code: 'VALIDATION' });
  });

  it('updates restaurant and returns success when valid and admin', async () => {
    mockedRequireAdmin.mockResolvedValue(GUARD_OK);
    const updated = { id: 'some-id', name_es: 'Actualizado' };
    const chain = makeChain({ data: updated, error: null });
    mockedCreateClient.mockResolvedValue(makeClient(chain));

    const result = await updateRestaurant('some-id', { name_es: 'Actualizado' });
...
describe('deleteRestaurant', () => {
  ...
  it('returns UNAUTHORIZED when not admin', async () => {
    ...
    const result = await deleteRestaurant('some-id');

    expect(result).toEqual(GUARD_FAIL);
    expect(mockedCreateClient).not.toHaveBeenCalled();
  });

  it('deletes restaurant and returns success when admin', async () => {
    mockedRequireAdmin.mockResolvedValue(GUARD_OK);
...
describe('togglePublish', () => {
  ...
  it('can unpublish a restaurant', async () => {
    mockedRequireAdmin.mockResolvedValue(GUARD_OK);
    const chain = makeChain({ data: null, error: null });
    mockedCreateClient.mockResolvedValue(makeClient(chain));
